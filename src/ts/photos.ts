...
class Waterfall {
  ...
  private static totalPage: number = 1;

  private static initPaginationState(container: HTMLElement): void {
    this.currentPage = Number(container.getAttribute("data-page") || 1) + 1;
    this.totalPage = Number(container.getAttribute("data-total-page") || 1);
  }
  ...
  private static sortElements(elements: HTMLElementWithPosition[]): void {
    elements.sort((a, b) => {
      return this.getBottom(a) === this.getBottom(b)
        ? this.getLeft(b) - this.getLeft(a)
        : this.getBottom(b) - this.getBottom(a);
    });
  }
  ...
  public static applyLayout(container: HTMLElement | string): void {
    ...
    for (n = 1; n < elements.length; n++) {
      ...
      const fitsNextTo =
        this.getRight(prevElement) + this.getClientWidth(currElement) <= this.getClientWidth(container);
  ...
  public static initInfiniteScroll(containerSelector: string): void {
    const observerForLoading = new IntersectionObserver(
      (entries) => {
  ...
  // 对外暴露统一初始化方法（安全调用）
  public static initIfExist(): void {
    const selector = ".gallery-photos";
    const container = document.querySelector(selector) as HTMLElement;
    if (!container) return;

    this.initPaginationState(container);
    this.initLazyLoadAndLayout(container);
    this.initInfiniteScroll(selector);
  }
}

export function initWaterfallIfExist() {
  Waterfall.initIfExist();
}
